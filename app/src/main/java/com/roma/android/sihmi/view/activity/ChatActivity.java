...
//        toolbar.setTitle(getIntent().getStringExtra("nama").toUpperCase());
    ...
    private void initAll(){
        ...
        Chatting thisChatting = chatingDao.getChatingById(otherUser.get_id());
        if (thisChatting != null) {
            thisChatting.setUnread(0);
            chatingDao.insertChating(thisChatting);
        }
    ...
    private class readMessageAsync extends AsyncTask<DataSnapshot, Void, List<Chat>>{
        ...
        protected List<Chat> doInBackground(DataSnapshot... dataSnapshots) {
            for (DataSnapshot snapshot : dataSnapshots[0].getChildren()){
                ...
                if (chat != null && chat.getReceiver() != null && chat.getSender() != null && (chat.getReceiver().equals(myuser) && chat.getSender().equals(otheruser) ||
    ...
    private void checkRvCount() {
        if (adapter != null) {
            int rvChatCount = adapter.getItemCount();
            stateView.checkState(rvChatCount);
    ...
    private void setAdapter(List<Chat> list){
        this.list = list;
        adapter = new RoomChatAdapter(this, list, 1, chat ->
            ...
            UploadFile.uploadFileToServer(type, filePath, new Callback<UploadFileResponse>() {
                ...
                public void onResponse(@NonNull Call<UploadFileResponse> call, @NonNull Response<UploadFileResponse> response) {
                    if (response.isSuccessful()) {
                        ...
                        if (body != null) {
                            Log.d("UPLOAD FOTO", "onResponse: activityResult " + body.getData().get(0).getUrl());
                            if (response.isSuccessful()) {
